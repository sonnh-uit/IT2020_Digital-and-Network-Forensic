...
def update_progress(frame, num_frames):
    progress = (frame / float(num_frames)) * 100
    sys.stdout.write('\rProgress: [{0}] {1}%'.format('>' * int(math.ceil(progress / 10)), progress))
    sys.stdout.flush()
...
# container_outvid = cv2.VideoWriter(args["output_video"],cv2.VideoWriter_fourcc('H','F','Y','U'), 15, (224,224))
container_outvid = cv2.VideoWriter('results/forensic-sf-real.avi',cv2.VideoWriter_fourcc('H','F','Y','U'), 15, (224,224))
...
while True:
        ...
        if frame % 4 == 0  :
            ...
            #    container_outvid_mp4.write(coverout[i][..., ::-1])
